...
type packer interface {
	...
	PackShapedPacket(onlyAck bool, now time.Time, v protocol.VersionNumber, maxPacketSize protocol.ByteCount) (shortHeaderPacket, *packetBuffer, error)
...
// PackShapedPacket is a modification of PackPacket for shaped traffic
func (p *packetPacker) PackShapedPacket(onlyAck bool, now time.Time, v protocol.VersionNumber, maxPacketSize protocol.ByteCount) (shortHeaderPacket, *packetBuffer, error) {
	sealer, err := p.cryptoSetup.Get1RTTSealer()
	if err != nil {
		return shortHeaderPacket{}, nil, err
	}
	...
	kp := sealer.KeyPhase()
	buffer := getPacketBuffer()
	paddingLen := maxPacketSize - pl.length - hdrLen - protocol.ByteCount(sealer.Overhead())

...
type packer interface {
	...
	PackShapedPacket(onlyAck bool, maxPacketSize protocol.ByteCount) (*packedPacket, error)
...
func (p *packetPacker) PackShapedPacket(onlyAck bool, maxPacketSize protocol.ByteCount) (*packedPacket, error) {
	sealer, err := p.cryptoSetup.Get1RTTSealer()
	if err != nil {
		return nil, err
	}
	hdr, payload := p.maybeGetShortHeaderPacket(sealer, maxPacketSize, onlyAck, true)
	if payload == nil {
		return nil, nil
	}
	buffer := getPacketBuffer()
	paddingLen := maxPacketSize - payload.length - hdr.GetLength(p.version) - protocol.ByteCount(sealer.Overhead())
	cont, err := p.appendPacket(buffer, hdr, payload, paddingLen, protocol.Encryption1RTT, sealer, false)
	if err != nil {
		return nil, err
	}
	return &packedPacket{
